...
WORKDIR /src
COPY ["DLP.Lambda.csproj", "DLP.Lambda/"]
RUN dotnet restore "DLP.Lambda/DLP.Lambda.csproj"

WORKDIR "/src/DLP.Lambda"
COPY . .
RUN dotnet build "DLP.Lambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "DLP.Lambda.csproj" \
...
Execute unit tests
```
cd "DLP.Lambda/test/DLP.Lambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "DLP.Lambda/src/DLP.Lambda"
